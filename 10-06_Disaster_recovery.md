...
<ins>*Сравнение DRaaS с моделью «резервное копирование как услуга» (BaaS)*</ins>
...
**Active-Active**
Кластер *"активный-активный"* обычно состоит как минимум из двух узлов, на каждом из которых одновременно запущены службы одного типа. Основная цель такого кластера — добиться балансировки нагрузки. Балансировка нагрузки распределяет рабочие нагрузки по всем узлам, чтобы предотвратить перегрузку любого отдельного узла. Поскольку для обслуживания доступно больше узлов, также будет заметно улучшение пропускной способности и времени отклика. В случе отказа одного из серверов нагрузка будет снижен уровень сервиса (время отклика, пропускная способность и тд), но не доступность. У IT службы есть время на устранение проблем. 

**Active-Passive**
Как следует из названия *"активный-пассивный"*, не все узлы будут активными. В случае двух узлов, например, если первый узел уже активен, второй узел должен быть пассивным или находиться в режиме ожидания.
...
> Т.о. разница между DRaaS, BaaS, Active-Active, Active-Passive основана на разных подходах в части обеспечения сохранности данных и сервисов.  
> DRaaS - использует технологии виртуализации и позволяет надежно сохранять не только данные, но и инфраструктуру в целом.  
> BaaS — это технология автоматизированного резервного копирования и восстановления критически важных данных.
> Active-Active, Active-Passive - технология, построенная на кластеризации, и позволяет получить отказоустойчивую систему высокой доступности и непрерывности сервиса. 
> Конечно, в современных системах, для обеспечения непрерывности оказания услуги и сохранности данных, применяется комплекс методов и как правило компании не ограничиваются только одним вышеописанным подходом.
...
При текущем развитии облачных решение наиболее оптимальным решением будет - резервирование и восстановление из облака (BaaS)
Почему Baas:
* Расположение данных в другой геолокации
* Быстрота развертования
...
План резервирования: 
Системный  													Диск
диск <img src = "pics/1006/sysdisk.png" width = 50%> 		данных <img src = "pics/1006/datadisk.png" width = 50%> 
